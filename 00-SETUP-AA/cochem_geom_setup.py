...
class CoChemSetupError(RuntimeError):
    pass

class GeomEnvironmentOrchestrator:
    def __init__(self, workspace_name: str, config_path: str | Path | None = None, headless: bool = False) -> None:
    ...
    def _load_system_config(self) -> None:
        ...
        if not self.config_path.exists():
            ...
            self.hardware_profile = {
                "cpu_cores": max(1, (cpu_count - 1) if cpu_count is not None else 1),
                "system_ram_gb": 8.0,
        ...
        try:
            with open(self.config_path, 'r') as f:
                ...
                cpu_count = os.cpu_count()
                self.hardware_profile = {
                    "cpu_cores": hw.get("cpu_cores", max(1, (cpu_count - 1) if cpu_count is not None else 1)),
                    "system_ram_gb": hw.get("system_ram_gb", 8.0),
            ...
            raise CoChemSetupError("Corrupted cochem_system_config.json detected. Aborting.")
    ...
    def _initialize_swmr_lake(self) -> Path:
        ...
        try:
            ...
            try:
                with h5py.File(h5_path, 'r', libver='latest', swmr=True) as f_read:
                    pass
            except OSError:
            ...
            raise CoChemSetupError(f"Critical error initializing HDF5 lake: {e}")
