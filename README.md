# CoChem-GEOM: Molecular Triage and Eckart Alignment
...
The ingestion and structural sanitization module.
...
Constructs a distance-thresholded molecular graph $G(V, E)$, detects pathological van der Waals clashes, and rigorously aligns the molecular frame to the standard Eckart conditions by diagonalizing the mass-weighted Gram matrix.
...
```bash
cochem-geom --sanitize input.xyz --eckart
```
...
pipeline.run(target_module="CoChem-GEOM")
```
